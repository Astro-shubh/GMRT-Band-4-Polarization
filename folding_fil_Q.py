...
if (len(sys.argv) < 5):
    ...
    exit(1)
filename='Read_stokes_Q_dedisp_fcosed.dat'
fn=open(filename,'rb')
fp=open("profile_Q.txt","w")
fp1=open("profile1_Q.txt","w")
...
plt.colorbar(c)
plt.savefig('Profile_Q2D.png')
plt.clf()
prof=np.mean(profile[10:110:1], axis=0)
plt.plot(phase,prof)
plt.savefig('profile_Q1D.png')
plt.clf()
plt.plot(phase,profile1)
plt.savefig('profile_Q1Da.png')
for r in profile1:
    fp1.write(str(r)+"\n")
fn.close()
fp.close()
