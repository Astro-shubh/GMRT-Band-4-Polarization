...
if (len(sys.argv) < 5):
    ...
    exit(1)
filename='Read_stokes_I_dedisp_fcosed.dat'
fn=open(filename,'rb')
fp=open("profile_I.txt","w")
fp1=open("profile1_I.txt","w")
...
plt.colorbar(c)
plt.savefig('Profile_I2D.png')
plt.clf()
prof=np.mean(profile[10:110:1], axis=0)
plt.plot(phase,prof)
plt.savefig('profile_I1D.png')
plt.clf()
plt.plot(phase,profile1)
plt.savefig('profile_I1Da.png')
for r in profile1:
    fp1.write(str(r)+"\n")
fn.close()
fp.close()
